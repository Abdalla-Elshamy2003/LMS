            ...
            <motion.div initial={{ opacity: 0, x: -20 }} animate={{ opacity: 1, x: 0 }} className="lg:col-span-3">
              {user && !asStudent ? (
                ...
                <form onSubmit={submit} className="card space-y-4 p-7">
                  ...
                  {!user && <p className="text-center text-sm text-ink-400">عندك حساب على منارة؟ <Link to="/login" className="font-bold text-brand-600 hover:underline">سجّل دخولك</Link> وارجع هنا — وحسابك نفسه هيتضافله كل المدرسين.</p>}
                </form>
              )}
            </motion.div>
          </div>
