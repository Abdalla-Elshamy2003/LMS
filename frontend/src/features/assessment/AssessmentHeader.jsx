...
export default function AssessmentHeader({ title, description, stats = [] }) {
  return <section className="overflow-hidden rounded-3xl border border-ink-200 bg-white">
    <div className="flex items-start gap-4 p-6 sm:p-8">
      ...
      <div><p className="text-xs font-bold tracking-wide text-brand-700">دروس / مساحة التقييم</p><h2 className="mt-2 text-2xl font-black text-ink-900">{title}</h2><p className="mt-2 max-w-2xl text-sm leading-7 text-ink-500">{description}</p></div>
    </div>
